{lib, ...}: let
  inherit (lib) mkEnableOption mkOption types literalExpression mkRemovedOptionModule mkRenamedOptionModule;
in {
  imports =
    [
  ...
  options.vim.presence.neocord = {
    ...
    setupOpts = lib.nvim.types.mkPluginSetupOption "neocord" {
      logo = mkOption {
